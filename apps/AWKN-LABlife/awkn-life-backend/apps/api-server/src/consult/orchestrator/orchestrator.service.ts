...
import type { FactLedger } from './react-engine.service';
...
import { enforceRiskLevel } from './score-engine';
// 任务 2.3: 注入 K线/潮汐服务
...
export class XuanxueOrchestratorService {
  ...
  constructor(
    ...
    // P0-4 Step 5: 5 层输出定向修复器（注入可选，避免循环依赖）
    @Optional() private readonly layerRepair?: LayerRepairService,
  ) {}
  ...
  private async executeL2Pipeline(
    ...
    if (schedulingDecision.mode === 'quick_read' || (schedulingDecision.secondaryAgent && schedulingDecision.mode === 'deep_consult')) {
      ...
      if (schedulingDecision.mode === 'deep_consult') {
        ...
        // P0-3: 构造结构化 FactLedger，避免 ReAct 反复追问已存在字段
        const factLedger: FactLedger = this.buildFactLedger(data, calcResult, memorySummary, parallelResult);
        ...
        this.logger.log(`[Orchestrator] ReAct done=${reactResult.done} iters=${reactResult.totalIters} reason=${reactResult.reason} blockedClarifications=${reactResult.blockedClarifications?.length || 0}`);
      }
      ...
      if (pipelineV2Enabled) {
        // P0-4 Step 6: 用 Safe 版本（不抛错，返回 missingLayers 供修复器使用）
        // 老 evaluateFiveLayers() 保留 P1-3 throw 行为，供测试和老调用方使用
        fiveLayerValidation = this.qualityGate.evaluateFiveLayersSafe(renderedOutput);
        this.logger.log(`[Orchestrator] V2 validation (Safe): passed=${fiveLayerValidation.passed} score=${fiveLayerValidation.score} identity=${fiveLayerValidation.identityScore} repairable=${(fiveLayerValidation as any).repairable}`);

        // P0-4 Step 6: 质量门标为 repairable 时，调用定向修复器补全缺失层
        const safeValidation = fiveLayerValidation as any;
        if (safeValidation.repairable && safeValidation.missingLayers && this.layerRepair && safeValidation.parsedLayers) {
          this.logger.warn(
            `[Orchestrator] P0-4 5 层输出不完整，缺失 ${safeValidation.missingLayers.length} 层，调用修复器`,
          );
          try {
            const repairedLayers = await this.layerRepair.repairMissingLayers(
              safeValidation.missingLayers,
              safeValidation.parsedLayers,
              data.question,
            );
            // 把修复结果合入 zhangbanshanOutput.five_layers（保留已有层，覆盖补全层）
            const existing = (zhangbanshanOutput as any).five_layers || {};
            (zhangbanshanOutput as any).five_layers = {
              clause: existing.clause?.trim() || repairedLayers.clause,
              halfMountain: existing.halfMountain?.trim() || repairedLayers.halfMountain,
              detail: existing.detail?.trim() || repairedLayers.detail,
              cost: existing.cost?.trim() || repairedLayers.cost,
              nextAction: existing.nextAction?.trim() || repairedLayers.nextAction,
            };
            this.logger.log(
              `[Orchestrator] P0-4 修复器补全完成，记录于 recordId=${data.recordId}`,
            );
            // 标记 quality gate 通过（修复器补全后 5 层齐全）
            fiveLayerValidation = {
              ...fiveLayerValidation,
              passed: true,
            };
          } catch (repairErr) {
            // 修复器异常不阻断：原有兜底填充 L620-L628 会兜住
            this.logger.warn(
              `[Orchestrator] P0-4 修复器异常: ${(repairErr as Error).message}，依赖原有兜底`,
            );
          }
        }
      }
      ...
      // A-S2 (2026-07-06): 规则强制 riskLevel — 主链集成 enforceRiskLevel
      // 根因: enforceRiskLevel 已实现但全仓库无调用点, LLM 自评 riskLevel 漂移
      // 修复: 扫描 zhangbanshanOutput.jiShenSignals, 调用 enforceRiskLevel 强制覆盖
      if (zhangbanshanOutput.jiShenSignals && zhangbanshanOutput.jiShenSignals.length > 0) {
        const ruleRiskLevel = enforceRiskLevel(zhangbanshanOutput.jiShenSignals);
        const llmSelfEval = zhangbanshanOutput.llmSelfEvalRiskLevel;
        if (llmSelfEval && llmSelfEval !== ruleRiskLevel) {
          this.logger.log(`[Orchestrator] riskLevel 规则强制: LLM 自评=${llmSelfEval} → 规则=${ruleRiskLevel}（命中信号：${zhangbanshanOutput.jiShenSignals.join('、')}）`);
        }
        (zhangbanshanOutput as any).ruleRiskLevel = ruleRiskLevel; // 写入规则强制结果，前端/测试可读取
      }
      ...
      // A-S1 (2026-07-06): 扩展 inferAndPersist 触发入口 — 问事完成时触发 LLM 状态推导
      // 根因: inferAndPersist 原唯一入口是 POST /consult/save, 用户不保存则永远不触发
      // 修复: 在问事完成（parallel completed）后立即触发, fire-and-forget 不阻塞
      // 频率限制: tideInferenceService 内部按 stateSnapshot.updatedAt 24h 去重（见 inferAndPersist 实现）
      if (data.userId && this.tideInferenceService) {
        this.tideInferenceService.inferAndPersist(data.userId).catch((err) => {
          this.logger.warn(`[Orchestrator] A-S1 inferAndPersist failed for ${data.userId}: ${(err as Error).message}`);
        });
      }
      return;
    }
  ...
  /**
   * P0-3: 构造结构化 FactLedger
   * 从 OrchestratorInput + calcResult + memorySummary + parallelResult 提取已确认事实
   * 已存在字段将在 ReAct 引擎中阻断重复追问
   */
  private buildFactLedger(
    data: OrchestratorInput,
    calcResult: Record<string, unknown> | null,
    memorySummary: string,
    parallelResult: { primary?: any; secondary?: any },
  ): FactLedger {
    ...
    if (data.birthInfo) {
      ...
      factLedger.birthInfo = {
        year: b.year,
        month: b.month,
        day: b.day,
        hour: b.hour,
        gender: b.gender,
        formatted: `${b.year}年${b.month}月${b.day}日 ${b.hour}时 ${b.gender === 'male' ? '男' : b.gender === 'female' ? '女' : b.gender}`,
      };
    }

    // 2. 排盘结果 + 当前大运 + 当前流年（从 calcResult.calcData 提取）
    if (calcResult) {
      const cd = (calcResult as any).calcData;
      if (cd) {
        if (cd.yearPillar) {
          const pillars = `年柱${cd.yearPillar} 月柱${cd.monthPillar || ''} 日柱${cd.dayPillar || ''} 时柱${cd.hourPillar || ''}`;
          factLedger.chartSnapshot = {
            yearPillar: cd.yearPillar,
            monthPillar: cd.monthPillar,
            dayPillar: cd.dayPillar,
            hourPillar: cd.hourPillar,
        ...
        // 当前大运（取最后一条或根据 birthYear 匹配）
        if (cd.daYun && Array.isArray(cd.daYun) && cd.daYun.length > 0) {
          let currentDaYun: any;
          if (data.birthInfo?.year) {
            const age = 2026 - data.birthInfo.year;
            currentDaYun = cd.daYun.find((d: any) => age >= d.startAge && age <= d.endAge);
          }
          if (!currentDaYun) currentDaYun = cd.daYun[cd.daYun.length - 1];
          if (currentDaYun) {
            factLedger.currentDaYun = {
              startAge: currentDaYun.startAge,
              endAge: currentDaYun.endAge,
              gan: currentDaYun.gan,
              zhi: currentDaYun.zhi,
              formatted: `${currentDaYun.gan || ''}${currentDaYun.zhi || ''}运 (${currentDaYun.startAge || '?'}-${currentDaYun.endAge || '?'}岁)`,
            };
          }
        }

        // 当前流年（匹配 year=2026 或取第一条）
        if (cd.liuNian && Array.isArray(cd.liuNian) && cd.liuNian.length > 0) {
          const currentLiuNian = cd.liuNian.find((ln: any) => ln.year === 2026) || cd.liuNian[0];
          if (currentLiuNian) {
            factLedger.currentLiuNian = {
              year: currentLiuNian.year,
              gan: currentLiuNian.gan,
              zhi: currentLiuNian.zhi,
              formatted: `${currentLiuNian.year || 2026}${currentLiuNian.gan || ''}${currentLiuNian.zhi || ''}年`,
            };
          }
        }
      }
    ...
    // 3. 用户背景（从 memorySummary 提取）
    if (memorySummary && memorySummary.trim()) {
      factLedger.userContext = {
        background: memorySummary.slice(0, 300),
      };
    }

    // 4. 并行 Agent 结果
    if (parallelResult?.primary || parallelResult?.secondary) {
      factLedger.parallelResults = {
        primary: parallelResult.primary
          ? (parallelResult.primary.summary_line + ' ' + parallelResult.primary.summary_body).slice(0, 300)
          : undefined,
        secondary: parallelResult.secondary
          ? (parallelResult.secondary.summary_line + ' ' + parallelResult.secondary.summary_body).slice(0, 300)
          : undefined,
      };
    }

    this.logger.log(
      `[Orchestrator P0-3] FactLedger 构造完成: ` +
      `birthInfo=${!!factLedger.birthInfo} ` +
      `chartSnapshot=${!!factLedger.chartSnapshot} ` +
      `currentDaYun=${!!factLedger.currentDaYun} ` +
      `currentLiuNian=${!!factLedger.currentLiuNian} ` +
      `userContext=${!!factLedger.userContext?.background} ` +
      `parallelResults=${!!factLedger.parallelResults?.primary}`,
    );
