...
export interface ZhangbanshanOutput {
  ...
  /** A-S2 (2026-07-06): LLM 自评分值（1-100），parseThreeStageOutput 解析【分值】段 */
  llmSelfEvalScore?: number;
  /** A-S2: LLM 自评风险等级（low/medium/high），规则强制前的原始值 */
  llmSelfEvalRiskLevel?: 'low' | 'medium' | 'high';
  /** A-S2: LLM 输出的 3 条得分依据 */
  scoreEvidence?: string[];
  /** A-S2: 从 LLM 输出提取的忌神信号（用于 enforceRiskLevel） */
...
export class ZhangbanshanSchedulerService {
  ...
  private parseThreeStageOutput(raw: string, decision: SchedulingDecision, costWarningType?: string): ZhangbanshanOutput {
    ...
    // A-S2 (2026-07-06): 解析【分值】段 — 提取 LLM 自评 score/riskLevel/evidence + jiShenSignals
    // 根因: parseThreeStageOutput 原只解析 4 段(判断/前提/代价/推理轨迹), 【分值】段全丢
    // 修复: 新增【分值】段正则, 提取 score/riskLevel/evidence, 从全文扫描 jiShenSignals
    const scoreSectionMatch = raw.match(/【分值】\s*([\s\S]*?)(?=【观察期】|【红线】|【三窗口】|【落一句最实在的话】|$)/);
    if (scoreSectionMatch) {
      const scoreSection = scoreSectionMatch[1];
      // 提取分值（支持 "58/100"、"分值：58"、"58 分" 等格式）
      const scoreNumMatch = scoreSection.match(/(\d{1,3})\s*\/\s*100|分值[：:]\s*(\d{1,3})|(\d{1,3})\s*分/);
      if (scoreNumMatch) {
        const scoreNum = parseInt(scoreNumMatch[1] || scoreNumMatch[2] || scoreNumMatch[3], 10);
        if (scoreNum >= 1 && scoreNum <= 100) {
          output.llmSelfEvalScore = scoreNum;
        }
      }
      // 提取风险等级
      const riskMatch = scoreSection.match(/(low|medium|high)/i);
      if (riskMatch) {
        const r = riskMatch[1].toLowerCase();
        if (r === 'low' || r === 'medium' || r === 'high') {
          output.llmSelfEvalRiskLevel = r as 'low' | 'medium' | 'high';
        }
      }
      // 提取 3 条得分依据（按行分割，过滤空行和标签行）
      const evidenceLines = scoreSection
        .split('\n')
        .map(l => l.trim())
        .filter(l => l && !l.match(/^\d+\s*\/\s*100$/) && !l.match(/^分值[：:]/) && !l.match(/^(low|medium|high)$/i))
        .slice(0, 3);
      if (evidenceLines.length > 0) {
        output.scoreEvidence = evidenceLines;
      }
    }

    // A-S2: 从全文扫描忌神信号（enforceRiskLevel 的输入）
    // 信号来源: LLM 输出的【解读层】+【分值】段, 命中 HIGH/MEDIUM RISK SIGNALS 关键词
    ...
    const hitSignals = allSignals.filter(s => raw.includes(s));
    if (hitSignals.length > 0) {
      output.jiShenSignals = hitSignals;
    }
