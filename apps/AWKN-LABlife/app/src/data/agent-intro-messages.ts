/**
 * Agent Introduction Messages · 算法话术过渡语（三段式）
 *
 * 每种算法调用前，张半山说三段话术：
 ...
 * - setup：结果预告，设定预期
 *
 * 与后端 agent-intro-messages.ts 保持一致
 */

export type AgentType = 'liuren' | 'qimen' | 'ziping' | 'ziwei' | 'liuyao' | 'quming';
...
export const AGENT_INTRO_MESSAGES: Record<AgentType, AgentIntroStages> = {
  liuren: {
 ...
 */
export function getAgentIntro(agent: string): AgentIntroStages | null {
  return AGENT_INTRO_MESSAGES[agent as AgentType] || null;
}

/**
 * 默认 fallback（防止未知算法类型）
 */
export const FALLBACK_INTRO: AgentIntroStages = {
  opening: '我看了你的盘。',
  process: '这件事，我从头说——',
  setup: '我说完之后，你有问题再问。',
};
